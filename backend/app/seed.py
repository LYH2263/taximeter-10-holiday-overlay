...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS holiday_factors(id INTEGER PRIMARY KEY, service_date TEXT NOT NULL, factor REAL NOT NULL, active INTEGER NOT NULL DEFAULT 1, created_at TEXT, updated_at TEXT);
    """)
    if conn.execute("SELECT COUNT(*) c FROM tariff").fetchone()["c"] == 0:
        ...
        conn.execute("INSERT INTO holiday_factors(service_date,factor,active,created_at,updated_at) VALUES ('2026-10-01',1.5,1,datetime('now'),datetime('now'))")
